...
from fastdraw.db import migration
from fastdraw.db.sqlalchemy import migrate_repo
from fastdraw.db.sqlalchemy import utils as db_utils
from fastdraw.openstack.common.db.sqlalchemy import test_migrations
...
class TestFastdrawMigrations(test_migrations.BaseMigrationTestCase,
    ...
    def __init__(self, *args, **kwargs):
        super(TestFastdrawMigrations, self).__init__(*args, **kwargs)

        self.DEFAULT_CONFIG_FILE = os.path.join(
            os.path.dirname(__file__), 'test_migrations.conf')
        # Test machines can set the FASTDRAW_TEST_MIGRATIONS_CONF variable
        # to override the location of the config file for migration testing
        self.CONFIG_FILE_PATH = os.environ.get(
            'FASTDRAW_TEST_MIGRATIONS_CONF', self.DEFAULT_CONFIG_FILE)
    ...
    def setUp(self):
        ...
        super(TestFastdrawMigrations, self).setUp()
        ...
        if self.migration_api is None:
            temp = __import__(
                'fastdraw.openstack.common.db.sqlalchemy.migration',
    ...
    def _check_001(self, engine, data):
        resource = db_utils.get_table(engine, 'resources')

        cols = [
            'id', 'created_at', 'project_id', 'resource_id',
            'resource_metadata', 'updated_at', 'user_id']

        for c in cols:
            self.assertIn(c, resource.c)

        resource.insert()

    def _post_downgrade_001(self, engine):
        self.assertRaises(
            exc.NoSuchTableError, db_utils.get_table, engine, 'resources')
